        ...
        {activeTab === 'synthid' && (
          <div className="p-6 space-y-6">
            ...
            {summary?.synthid ? (() => {
                ...
                // High AI confidence (≥ 0.80) or watermark → full red alert
                const isHighAI = wmFound || conf >= 0.80;
                // Only use explicit AI-GEN label at 95%+
                ...
                const confColor = isHighAI
                  ...
                  : 'text-green-700 dark:text-green-400';
                const barColor = conf >= 0.80 ? 'bg-red-500' : conf >= 0.50 ? 'bg-amber-400' : 'bg-green-500';
                const tierLabel = conf >= 0.80 ? 'HIGH' : conf >= 0.50 ? 'MEDIUM' : 'LOW';
